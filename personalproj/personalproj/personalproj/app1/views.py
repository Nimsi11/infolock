...
def family(request):
    if request.method == "POST":
        ...
        e = request.POST["name5"]
        f = request.POST["name6"]
        g = familytable(Name=a, father_name=b, mother_name=c, marital_status=d, spouse_name=e, number_of_children=f)
        g.save()
        return redirect('submit')

    return render(request,"familyform.html")

def familydata(request):
    x = usertable.objects.get(id=request.session["member_id"])
    print(x.Name)
    c = familytable.objects.filter(Name=x.Name)

    if request.method == "POST":
        c.Name = request.POST["name1"]
        c.father_name = request.POST["name2"]
        c.mother_name = request.POST["name3"]
        c.marital_status = request.POST["name4"]
        c.spouse_name = request.POST["name5"]
        c.number_of_children= request.POST["name6"]
        c.save()

    return render(request, "familydata.html", {"familykey": c})
...
def familyedit(request,id35):
    c = familytable.objects.get(id=id35)
    if request.method == "POST":
        c.Name = request.POST["name1"]
        c.father_name = request.POST["name2"]
        c.mother_name = request.POST["name3"]
        ...
        c.save()
        return redirect('update')
    return render(request,"familyedit.html", {'key9':c})

def famdlt(request,id36):
    c=familytable.objects.get(id=id36)
    c.delete()
    return redirect('deleted')
